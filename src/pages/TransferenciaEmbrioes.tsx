...
// Interface para hist├│rico de TE
...
// Interface para sess├úo de TE agrupada
interface SessaoTE {
  id: string;
  fazenda_nome: string;
  fazenda_id: string;
...
export default function TransferenciaEmbrioes() {
  ...
  // Estado de submiss├úo
  const [submitting, setSubmitting] = useState(false);

  // Estado para permitir 2┬║ embri├úo (caso raro ~3%)
  const [permitirSegundoEmbriao, setPermitirSegundoEmbriao] = useState(false);


  // Estados para dialog de sess├úo em andamento
  ...
  // Fun├º├Áes para o dialog de sess├úo em andamento
          ...
          try {
            ...
            if (transferenciasComEmbriao && transferenciasComEmbriao.length > 0) {
              const embriao = Array.isArray(transferenciasComEmbriao[0].embrioes)
                ...
                : transferenciasComEmbriao[0].embrioes as { lote_fiv_id: string | null; created_at: string | null } | null;
            ...
            console.error('Erro ao inferir pacote da sess├úo:', error);
          }
        }
  ...
  const handleDescartarSessao = async () => {
    ...
    setSessaoPendente(null);
    toast({
      title: 'Sess├úo descartada',
      description: 'Uma nova sess├úo ser├í iniciada.',
    });
  };
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Transfer├¬ncia de Embri├Áes (TE)"
        description="Destinar embri├Áes para receptoras sincronizadas"
      />
      ...
      {/* Dialog de Confirma├º├úo de Relat├│rio (Sess├úo Atual) */}
      ...
      {/* Dialog para Restaurar Sess├úo */}
      <AlertDialog open={showRestaurarDialog} onOpenChange={setShowRestaurarDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Sess├úo em Andamento Encontrada</AlertDialogTitle>
            <AlertDialogDescription>
              Voc├¬ tem uma sess├úo de transfer├¬ncia n├úo finalizada para a fazenda{' '}
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleRestaurarSessao}>Continuar Sess├úo</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
