...
interface ImportHistoryListProps {
    reportType: 'dg' | 'sexagem' | 'p1' | 'p2' | 'te' | 'aspiracao' | 'all';
}
...
export default function ImportHistoryList({ reportType }: ImportHistoryListProps) {
    const { imports, isLoading, revertImport, isReverting } = useReportImports();
    const [confirmRevert, setConfirmRevert] = useState<string | null>(null);

    const filteredImports = reportType === 'all'
        ? imports
        : imports.filter((imp) => imp.report_type === reportType);
            ...
            {isLoading ? (
                ...
                <Card>
                    <CardContent className="flex flex-col items-center gap-3 p-12">
                        ...
                        <p className="text-muted-foreground text-sm">Nenhum histórico encontrado para este serviço</p>
                    </CardContent>
                </Card>
            ) : (
