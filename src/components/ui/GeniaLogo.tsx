        ...
        <div className={cn("flex items-center justify-center", className)}>
            {showText && (
                <div className={cn("font-black tracking-tighter uppercase", isWhite ? "text-white" : "")} style={{ fontSize: size }}>
                    ...
                    <span className={cn(isWhite ? "text-white/80" : "")} style={!isWhite ? { background: 'var(--gradient-dna-text)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' } : {}}>.IA</span>
                </div>
            )}
        </div>
    );
